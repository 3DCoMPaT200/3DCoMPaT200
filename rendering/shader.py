...
class Shader:
    ...
    @staticmethod
    def apply_material(mesh_obj, mat_data, is_glass=False):
        mat = Shader.create_material_slot(mesh_obj)
        if is_glass:
            bsdf_node = Shader.get_bsdf(mat)
            if bsdf_node:
                # Assuming 'diffuse' is the key for the diffuse texture
                diffuse_texture = mat_data['diffuse'][0]  # assuming the image is the first item
                Shader.create_diffuse_node(diffuse_texture, mat, link_to_base=False)

                # Set glass properties
                bsdf_node.inputs['Roughness'].default_value = 0.0
                bsdf_node.inputs['Transmission Weight'].default_value = 1.0
                bsdf_node.inputs['IOR'].default_value = 1.45  # Typical IOR for glass

                # Link diffuse texture to Base Color of the Principled BSDF
                diffuse_node = Shader.create_shader_node(mat)
                diffuse_node.image = diffuse_texture
                mat.node_tree.links.new(diffuse_node.outputs['Color'], bsdf_node.inputs['Base Color'])

            else:
                print("Error: No Principled BSDF node found in the material.")
        else:
