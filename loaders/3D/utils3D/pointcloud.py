...
def map_meshes(obj, part_to_idx, part_remap, shape_id, part_to_mat_idx=None):
    """
    ...
    for mesh_name, part_raw in obj.graph.geometry_nodes.items():
        ...
        # part_name = clean_part(part_raw).split('.')[0]
        part_name = part_raw.split(".")[0]
        if part_name.split("_")[-1].isdigit():
            part_name = (
                "_".join(part_name.split("_")[:-1])
                if len(part_name.split("_")) > 2
                else part_name.split("_")[0]
            )
        ...
        if not isinstance(part_mesh.visual, trimesh.visual.TextureVisuals):
            part_mesh.visual = part_mesh.visual.to_texture()
