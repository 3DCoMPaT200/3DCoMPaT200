...
class Point_Transformer(nn.Module):
    def __init__(self, output_channels=40, normal_channel=True):
...
class Point_Transformer2(nn.Module):
    def __init__(self, output_channels=40, normal_channel=False):
        ...
        if normal_channel:
            ...
            channel = 3
        self.conv1 = nn.Conv1d(channel, 64, kernel_size=1, bias=False)
        self.conv2 = nn.Conv1d(64, 64, kernel_size=1, bias=False)
        self.bn1 = nn.BatchNorm1d(64)
        self.bn2 = nn.BatchNorm1d(64)
        self.gather_local_0 = Local_op(in_channels=67 + (channel - 3), out_channels=128)
        self.gather_local_1 = Local_op(
            in_channels=131 + (channel - 3), out_channels=256
        )
...
def get_model(output_channels=40, normal_channel=True):
    return Point_Transformer2(output_channels, normal_channel)
