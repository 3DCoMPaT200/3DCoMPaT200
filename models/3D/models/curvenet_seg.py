...
class get_model(nn.Module):
    def __init__(
        ...
        self.normal = normal_channel
        self.lpfa = LPFA(9, additional_channel, k=k, mlp_num=1, initial=True)

        # encoder
        # Replace the first layer with npoints for generalization purposes
        self.cic11 = CIC(
            npoint=npoints,
            radius=0.2,
            k=k,
        ...
        # if shape prior -> 301 otherwise 128+42 - 3
        self.up_cic2 = CIC(
            npoint=npoints,
            radius=0.2,
            k=k,
            in_channels=(128 + 64 + 64 + 3) + (num_shapes if self.shape_prior else 0),
            output_channels=256,
            bottleneck_ratio=4,
        )
        self.up_cic1 = CIC(
        ...
        self.conv1 = nn.Conv1d(512, 256, 1, bias=False)
    ...
    def forward(self, xyz, l=None):
        ...
        xyz, feat = self.up_cic1(xyz, x)

        x = F.leaky_relu(self.bn1(self.conv1(feat)), 0.2, inplace=True)
        se = self.se(x)
        x = x * se
        x = self.drop1(x)
        x = self.conv2(x)
