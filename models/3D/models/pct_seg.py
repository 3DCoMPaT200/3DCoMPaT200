...
class get_model(nn.Module):
    def __init__(
        self, num_classes, num_shapes=42, shape_prior=False, normal_channel=False
    ):
        ...
        if self.shape_prior:
            self.label_conv = nn.Sequential(
                nn.Conv1d(self.num_shapes, 64, kernel_size=1, bias=False),
                nn.BatchNorm1d(64),
                nn.LeakyReLU(negative_slope=0.2),
            )
    ...
    def forward(self, x, cls_label=None):
        ...
        feat = torch.cat((x, x_global_feature), 1)  # 1024 * 3 + 64
        x = self.relu(self.bns1(self.convs1(feat)))
        x = self.dp1(x)
        x = self.relu(self.bns2(self.convs2(x)))
        x = self.convs3(x)
        # pdb.set_trace()
        if self.part_num != 512:
