...
class PointTransformer(nn.Module):
    ...
    def forward(self, pts):
        ...
        feat = F.relu(self.bn1(ret))
        x = self.drop1(feat)
        x = self.drop2(F.relu(self.bn2(self.fc2(x))))
        x = self.fc3(x)
        if self.num_class != 512:
            x = F.log_softmax(x, -1)
        return x, None, None
...
class MaskTransformer(nn.Module):
    def __init__(self, **kwargs):
        ...
        self.num_heads = 6
        # print(f"[Transformer args] {config.transformer_config}", logger="dVAE BERT")
        # define the encoder
        self.encoder_dims = 256
        self.encoder = Encoder(encoder_channel=self.encoder_dims)
        # bridge encoder and transformer
    ...
    def _prepare_encoder(self, dvae_ckpt):
        ...
        print(
            f"[Encoder] Successful Loading the ckpt for encoder from {dvae_ckpt}",
            logger="dVAE BERT",
        )
    ...
    def _mask_center_rand(self, center, noaug=False):
        ...
        ratio = (
            random.random() * (self.mask_ratio[1] - self.mask_ratio[0])
            + self.mask_ratio[0]
        )
        bool_masked_pos = (
            (torch.rand(center.shape[:2]) < ratio).bool().to(center.device)
        )
    ...
    def _random_replace(self, group_input_tokens, bool_masked_pos, noaug=False):
        ...
        replace_mask = (
            (torch.rand(group_input_tokens.shape[:2]) < self.replace_pob)
        ...
        overall_mask = (
            (replace_mask + bool_masked_pos).bool().to(bool_masked_pos.device)
        )  #  True for flake input
        ...
        shuffled_group_input_tokens = flatten_group_input_tokens[idx].reshape(
            detached_group_input_tokens.size(0),
            detached_group_input_tokens.size(1),
            detached_group_input_tokens.size(2),
        )

        replace_mask = replace_mask.unsqueeze(-1).type_as(detached_group_input_tokens)
        replaced_group_input_tokens = (
            group_input_tokens * (1 - replace_mask)
            + shuffled_group_input_tokens * replace_mask
        )
    ...
    def forward(
        ...
        group_input_tokens = self.reduce_dim(group_input_tokens)
        # replace the point
        replaced_group_input_tokens, overall_mask = self._random_replace(
            group_input_tokens, bool_masked_pos.clone(), noaug=noaug
        )
        ...
        maksed_group_input_tokens = (
            replaced_group_input_tokens * (1 - w) + mask_token * w
        )
        # add pos embedding
        pos = self.pos_embed(center)
        # final input
...
class get_model(nn.Module):
    def __init__(self, num_class):
        ...
        self.moco_loss = False
        self.dvae_loss = False
        self.cutmix_loss = False

        self.return_all_tokens = False
        if self.return_all_tokens:
            print(f"[Point_BERT] Point_BERT calc the loss for all token ...")
        else:
            print(f"[Point_BERT] Point_BERT [NOT] calc the loss for all token ...")

        self.transformer_q = MaskTransformer()
        # self.transformer_q._prepare_encoder(self.config.dvae_config.ckpt)

        self.transformer_k = MaskTransformer()
        for param_q, param_k in zip(
            self.transformer_q.parameters(), self.transformer_k.parameters()
        ):
    ...
    def _prepare_dvae(self):
        ...
        print(f"[dVAE] Successful Loading the ckpt for dvae from {dvae_ckpt}")
    ...
    def forward_eval(self, pts):
        with torch.no_grad():
            neighborhood, center = self.group_divider(pts)
            cls_feature = self.transformer_q(
                neighborhood, center, only_cls_tokens=True, noaug=True
            )
            feat = F.relu(self.bn1(cls_feature))
            x = self.drop1(feat)
            x = self.drop2(F.relu(self.bn2(self.fc2(x))))
            x = self.fc3(x)
            if self.num_class != 512:
    ...
    def _mixup_pc(self, neighborhood, center, dvae_label):
        ...
        mixup_neighborhood = neighborhood * mixup_mask.unsqueeze(-1).unsqueeze(
            -1
        ) + neighborhood.flip(0) * (1 - mixup_mask.unsqueeze(-1).unsqueeze(-1))
        mixup_center = center * mixup_mask.unsqueeze(-1) + center.flip(0) * (
            1 - mixup_mask.unsqueeze(-1)
        )
        mixup_dvae_label = dvae_label * mixup_mask + dvae_label.flip(0) * (
            1 - mixup_mask
        )

        return (
            mixup_ratio.to(neighborhood.device),
            mixup_neighborhood,
            mixup_center,
            mixup_dvae_label.long(),
        )
    ...
    def forward(self, pts, noaug=False, **kwargs):
        if noaug:
            ...
            if self.return_all_tokens:
                q_cls_feature_true, logits = self.transformer_q(
                    neighborhood, center, return_all_tokens=self.return_all_tokens
                )  # logits :  N G C
            else:
                (
                    q_cls_feature_true,
                    ...
                    neighborhood, center, return_all_tokens=self.return_all_tokens
                )  # logits :  N' C where N' is the mask.sum()

            feat = F.relu(self.bn1(q_cls_feature_true))
            x = self.drop1(feat)
            x = self.drop2(F.relu(self.bn2(self.fc2(x))))
            x = self.fc3(x)
            if self.num_class != 512:
                x = F.log_softmax(x, -1)
            q_cls_feature = nn.functional.normalize(q_cls_feature_true, dim=1)
            ...
            (
                ...
                mixup_center,
                mix_dvae_label,
            ) = self._mixup_pc(neighborhood, center, dvae_label)
            if self.return_all_tokens:
                mixup_cls_feature, mixup_logits = self.transformer_q(
                    mixup_neighborhood,
                    mixup_center,
            ...
            with torch.no_grad():  # no gradient to keys
                ...
                k_cls_feature = self.transformer_k(
                    neighborhood, center, only_cls_tokens=True
                )  # keys: NxC
            ...
            if self.moco_loss:
                ...
                l_pos = torch.einsum(
                    "nc, nc->n", [q_cls_feature, k_cls_feature]
                ).unsqueeze(
                    -1
                )  # n 1
                l_neg = torch.einsum(
                    "nc, ck->nk", [q_cls_feature, self.queue.clone().detach()]
                )  # n k
                ...
                moco_loss = torch.tensor(0.0).to(pts.device)

            if self.dvae_loss:
                if self.return_all_tokens:
                    dvae_loss = self.loss_ce(
                        logits.reshape(-1, logits.size(-1)),
                        dvae_label.reshape(
                            -1,
                        ),
                    ) + self.loss_ce(
                        mixup_logits.reshape(-1, mixup_logits.size(-1)),
                        mix_dvae_label.reshape(
                            -1,
                        ),
                    )
                else:
                    dvae_loss = self.loss_ce(
                        flake_logits, dvae_label[mask]
                    ) + self.loss_ce(mixup_flake_logits, mix_dvae_label[mixup_mask])
            else:
                dvae_loss = torch.tensor(0.0).to(pts.device)

            if self.cutmix_loss:
                l_pos = torch.einsum(
                    "nc, mc->nm", [mixup_cls_feature, k_cls_feature]
                )  # n n
                l_neg = torch.einsum(
                    "nc, ck->nk", [mixup_cls_feature, self.queue.clone().detach()]
                )  # n k
                ce_logits = torch.cat([l_pos, l_neg], dim=1)
                ce_logits /= self.T
                labels = torch.arange(l_pos.shape[0], dtype=torch.long).to(pts.device)
                cutmix_loss = (
                    mixup_ratio * self.loss_ce_batch(ce_logits, labels)
                    + (1 - mixup_ratio) * self.loss_ce_batch(ce_logits, labels.flip(0))
                ).mean()
            else:
                cutmix_loss = torch.tensor(0.0).to(pts.device)
            # self._dequeue_and_enqueue(k_cls_feature)
            return x, moco_loss + dvae_loss, cutmix_loss
