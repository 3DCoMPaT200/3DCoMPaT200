...
class ModelNet(data.Dataset):
    ...
    def _get_item(self, index):
        if self.process_data:
            ...
            point_set = np.loadtxt(fn[1], delimiter=",").astype(np.float32)
            print('initial dimension of modelnet',point_set.shape)
            if self.uniform:
                point_set = farthest_point_sample(point_set, self.npoints)
            else:
                point_set = point_set[0 : self.npoints, :]
        ...
        # add a color of 0,0,0 to the pointset
        # point_set = np.concatenate((point_set, np.zeros((point_set.shape[0], 3))), axis=1)
        if self.use_height:
            self.gravity_dim = 1
            height_array = (
...
@DATASETS.register_module()
class CompatDataset(data.Dataset):
    def __init__(self, config):
        self.num_comps = config.NUM_COMPS
        self.img_path = "data/images/{}_{}.render.png"

        self.pc_path = "data/pointclouds/{}.npy"
        
        self.captions = json.load(open(config.CAP_PATH))

        self.parts = json.load(open(config.PARTS_PATH)) 
        self.mats = json.load(open(config.MATS_PATH))
        self.classes = json.load(open(config.CLASSES_PATH))
    ...
    def _get_shape_style_ids(self):
        shape_style_ids = []
        for  i, ids in enumerate(zip(self.shape_ids , self.style_ids)):
                style_id, shape_id = ids
                shape_style_ids.append(f"{shape_id}__{style_id}")
                self.shape2idx[f"{shape_id}__{style_id}"] = i
        return shape_style_ids
    ...
    def __getitem__(self, idx):
        ...
        shape_style_id = list(self.captions.keys())[idx]
        caption = self.captions[shape_style_id]

        pointcloud = np.load(self.pc_path.format(shape_style_id.replace('__','_')))
        pointcloud = torch.from_numpy(self.pc_norm(pointcloud))
        #create and tokenize the caption
        tokenized_captions = []
        tokenized_captions.append(self.tokenizer(caption))
        tokenized_captions = torch.stack(tokenized_captions)
        tokenized_captions = tokenized_captions.reshape(-1,77)
        # Read image data from tar file
        img_file = self.img_path.format(shape_style_id.replace('__','_'), np.random.randint(0, 8))
        image = pil_loader(img_file)
        image = self.train_transform(image)
        image = image.reshape(3, 224, 224)

        return shape_style_id, shape_style_id, tokenized_captions, pointcloud, image
    
@DATASETS.register_module()
class CompatDatasetVal(data.Dataset):
    def __init__(self, config):
        self.num_comps = config.NUM_COMPS
        self.img_path = "data/images/{}_{}.render.png"
    ...
    def __getitem__(self, idx):
        ...
        class_id = int(shape_style_id.split('_')[0], 16)
        class_name = self.classes[class_id]
        return pointcloud, class_id, class_name
...
class Dataset_3D:
    def __init__(self, args, tokenizer, dataset_type, train_transform=None):
        ...
        with open("./data/dataset_catalog.json", "r") as f:
            ...
            self.dataset_split = self.dataset_catalog[self.dataset_name][
                self.dataset_usage
            ]
