...
def get_args_parser():
    ...
    parser.add_argument('--pretrain_dataset_name', default='compat', type=str)
    parser.add_argument('--pretrain_dataset_prompt', default='compat', type=str)
    ...
    parser.add_argument('--comp_path', default='/ibex/project/c2106/Mahmoud/datasets/shards/test_captions.json', type=str)
    # Model
    ...
    parser.add_argument('--normal', action='store_true', help='eval 3d only')
...
def main(args):
    ...
    if args.evaluate_3d:
        zero_stats = test_recall_3d(args)
...
def test_recall_3d_core(data_path, model, tokenizer, args=None):
    batch_time = AverageMeter('Time', ':6.3f')
    top1 = AverageMeter('Acc@1', ':6.2f')
    top5 = AverageMeter('Acc@5', ':6.2f')
    with open(data_path) as f:
        captions = json.load(f)
    val_ids = list(captions.keys())#[:3]
    progress = ProgressMeter(
        len(val_ids),
        [batch_time, top1, top5],
        prefix='Test: ')
    ...
    val_pc = {k: np.load(f'/ibex/project/c2106/Mahmoud/datasets/shards/pointclouds/{k.replace("__","_")}.npy') for k in val_ids}


    with torch.no_grad():
        text_features = []
        for k in tqdm(val_ids, total=len(val_ids)):
            texts = captions[k]
            texts = tokenizer(texts).cuda(args.gpu, non_blocking=True)
            if len(texts.shape) < 2:
                texts = texts[None, ...]
        ...
        pc_features = []
        for k in tqdm(val_ids, total=len(val_ids)):
            pc = torch.from_numpy(pc_norm(val_pc[k])).cuda(args.gpu, non_blocking=True)
            pc = pc.unsqueeze(0).contiguous()

            pc_feature = utils.get_model(model).encode_pc(pc).squeeze(0)
        ...
        similarity_matrix = np.zeros((len(val_ids), len(val_ids)))
        for i, tid in tqdm(enumerate(val_ids), total=len(val_ids)):
            for j, pid in enumerate(val_ids):
                similarity_matrix[i, j] = cosine_similarity(
                    [text_features[i].cpu().numpy()], [pc_features[j].cpu().numpy()]
                )[0, 0]
        # Compute R1 and R5
        r1_count = 0
        r5_count = 0
        ...
        for i, tid in enumerate(val_ids):
            ...
            if val_ids[i] == val_ids[sorted_indices[0]]:
                r1_count += 1
            if val_ids[i] in [val_ids[sorted_indices[k]] for k in range(5)]:
                correct_mapping[val_ids[i]] = [val_ids[sorted_indices[k]] for k in range(5)]
                r5_count += 1
            else:
                wrong_mapping[val_ids[i]] = [val_ids[sorted_indices[k]] for k in range(5)]


        r1_score = r1_count / len(val_ids) * 100
        r5_score = r5_count / len(val_ids) * 100
        correct_path =  data_path.replace('.json', '_correct_mapping.json')
        wrong_path =  data_path.replace('.json', '_wrong_mapping.json')
        with open(correct_path, "w") as f:
            json.dump(correct_mapping, f)
        print('saved correct mapping in', correct_path)
        with open(wrong_path, 'w') as f:
            json.dump(wrong_mapping, f)
        print('saved false mapping in', wrong_path)
      
    progress.synchronize()
    print(f'R@1 {r1_score:.3f} R@5 {r5_score:.3f}')
    return {'R@1': r1_score, 'R@5': r5_score}


def test_recall_3d(args):
    ckpt = torch.load(args.test_ckpt_addr, map_location='cpu')
    state_dict = OrderedDict()
    for k, v in ckpt['state_dict'].items():
        state_dict[k.replace('module.', '')] = v
    ...
    tokenizer = SimpleTokenizer()
    data_path = args.comp_path
    # test_dataset = get_dataset(None, tokenizer, args, 'val')
    # test_loader = torch.utils.data.DataLoader(
    #     test_dataset, batch_size=args.batch_size, shuffle=False,
    #     num_workers=args.workers, pin_memory=True, sampler=None, drop_last=False
    # )
    results = test_recall_3d_core(data_path, model, tokenizer, args)
